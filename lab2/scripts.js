document.addEventListener("DOMContentLoaded", () => {
    ...
    function permutations(array, k, value){
        var m=[];
        var combinations = [];
        var indices = [];
            ...
            for(var i=0; i < len; i++){
                if(!indices[i]){
                    ...
                    if(level < k - 1){
                        ...
                        if (sha256(combinations.join('')) == value) {
                            result.innerHTML = combinations.join('');
    ...
    document.addEventListener("click", (event) => {
        ...
        while (att === true) {
            const resultStr = permutations(symbols, i, input.value);
            console.log(`${i} symbols checked`);
            i++;
        }
    });
});